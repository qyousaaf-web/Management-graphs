...
<style>
    ...
    .pdf-btn > button { background-color: #d32f2f; color: white; font-weight: bold; }
...
def generate_medical_report_pdf(patient_name, patient_cnic, records_df):
    buffer = io.BytesIO()
    ...
    if not records_df.empty:
        data = [["Visit Date", "Doctor", "Diagnosis", "Treatment", "Prescription"]]
    ...
    story.append(Paragraph("This report is confidential and intended for medical use only.", styles['Italic']))
...
# ================= MEDICAL RECORDS WITH PDF EXPORT =================
    ...
    st.subheader("🔍 Search Patient to Generate Report")
    ...
    # Show Records and PDF Button
    if not records_df.empty:
        st.subheader(f"Medical History - {patient_name}")
        st.dataframe(records_df, use_container_width=True)

        # Generate PDF and Download Button
        pdf_buffer = generate_medical_report_pdf(patient_name, search_cnic, records_df)
        st.markdown('<div class="pdf-btn">', unsafe_allow_html=True)
        st.download_button(
            label="📄 Download Patient Medical Report as PDF",
            data=pdf_buffer,
        ...
        if search_btn:
            st.warning("No medical records found for this CNIC.")
...
# ================= OTHER MODULES (Dashboard, Patients, Doctors, Appointments, Billings) =================
# (Keep your existing code for other tabs as in previous versions)
